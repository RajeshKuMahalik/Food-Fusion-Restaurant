...
const PlaceOrder = () => {
  ...
  const { getCartCountAmount, token, food_list, cartItems, setCartItems, url } = useContext(StoreContext);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      let orderData = {
        address: data,
        items: orderItems,
        amount: getCartCountAmount() + randomNum,
      }
        ...
        <div className='flex justify-center my-5'>
          <div className='text-2xl w-[40vw]'>
            ...
            <div className='flex flex-col gap-2 mt-2 text-sm'>
              ...
              <div className='flex justify-between'>
                <p>Delivey Fee =</p>
                <p className='lg:mr-80'>Rs : {getCartCountAmount() === 0 ? 0 : randomNum}</p>
              </div>
              <div className='flex justify-between'>
                <p>Total </p>
                <p className='lg:mr-80'>Rs : {getCartCountAmount() === 0 ? 0 : getCartCountAmount() + randomNum}</p>
              </div>
            </div>
          </div>
        </div>
